...
document.addEventListener("submit", function (e) {
  if (
    e.target &&
    (e.target.classList.contains("leadform") ||
      e.target.classList.contains("second-step-leadform"))
  ) {
    if (
      typeof iti != "undefined" &&
      e.target.getElementsByClassName("p-validate").length > 0 &&
      document.getElementsByClassName("phonevalidate")[0] !== undefined &&
      e.target.getElementsByClassName("phonevalidate")[0].value.length > 0 &&
      !iti[
        e.target
          .getElementsByClassName("p-validate")[0]
          .getAttribute("data-iti")
      ].isValidNumber()
    ) {
      e.preventDefault();
      ...
      .then(function (token) {
        ...
        e.target.childNodes.forEach(function (childNode) {
          try {
            if (
              ...
              childNode.type.toLowerCase() == "hidden" &&
              childNode.name.toLowerCase() == "gcaptcha"
            ) {
        ...
        if (e.target.classList.contains("lfcta")) {
          ...
          for (var ei = 0; ei < e.target.elements.length; ei++) {
            data[e.target.elements.item(ei).name] =
              e.target.elements.item(ei).value;
          }

          var response = function () {
            var bodyclass = e.target
              .getElementsByClassName("lfbtn")[0]
              .getAttribute("data-bodyclass");
            if (bodyclass != null) {
              document.getElementsByTagName("body")[0].classList.add(bodyclass);
            }
            var nextform = e.target
              .getElementsByClassName("lfbtn")[0]
              .getAttribute("data-nextformid");
            if (nextform == null) {
              var data = JSON.parse(this.responseText);
              if (data.redirect == true) {
                e.target.getElementsByClassName("lfmsg")[0].innerText =
                  "Redirecting...";
                e.target
                  .getElementsByClassName("lfmsg")[0]
              ...
              var data = JSON.parse(this.responseText);
              if (data.redirect == true) {
                nextform.setAttribute(
                  "action",
                  data.url.replace("/s/", "/go2/")
                );
...
window.setTimeout(function () {
  ...
  if (s_c_ut == 1 && window.location.href.includes("")) {
    $zopim(function () {
...
function setName(name) {
  if (!name.includes(lead_typing_name)) setCookie("name", name, 365);
...
style.innerHTML =
  ".grecaptcha-badge { left: 0 !important; width: 70px !important;bottom: 0px !important; }";
